...
$evaluation = mysqli_fetch_object(
  mysqli_query(
    $con,
    "SELECT * FROM evaluation WHERE evaluation_id='$_GET[id]'"
  )
);

$student = getUserById($evaluation->user_id);
...
<body>
          ...
          <div class="card mb-3">
            ...
            <div class="card-body">
              <form class="row g-3" id="ratingForm" method="POST" novalidate>
                ...
                <input type="text" name="evaluation_id" value="<?= $evaluation->evaluation_id ?>" hidden readonly>
                <table class="table table-stripe table-bordered">
                  ...
                  <tbody>
                    <?php
                    foreach (json_decode($evaluation->evaluation, true) as $index => $rating_detail) :
                      $total += $rating_detail["value"];
                    ?>
                      <tr>
                        <td><?= intval($index) + 1 . ". " . $rating_detail["title"] ?></td>
                        <?= generatedRadio(5, $rating_detail["name"], $rating_detail["value"]) ?>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
                  <caption style="caption-side:bottom">
                    <h4>
                      Total:
                      <label id="labTotal"><?= $total ?></label>
                    </h4>
                    <h4>
                      Mean:
                      <label id="interval"><?= number_format($total / count(json_decode($evaluation->evaluation, true)), 2) ?></label>
                    </h4>
                  </caption>
                </table>
                <?php if ($user->role == "admin") : ?>
                  <div class="card-footer d-flex justify-content-end">
                    <button type="button" class="btn btn-warning m-1" id="btnSubmit">Update</button>
  ...
  <script>
    ...
    const ratingBehavior = <?= $evaluation->evaluation ?>;
      ...
      if ($(`#ratingForm`).valid()) {
        ...
        $.post(
          `../../backend/nodes?action=saveEvaluation`, {
            ...
            evaluation_id: $("input[name='evaluation_id']").val()
          },
          (data, status) => {
